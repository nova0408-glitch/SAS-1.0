...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($rateLimiter->isLimitExceeded($client_ip, 'login')) {
        $auditLog->log('login_attempt', 'Rate limit exceeded', 'failed');
    ...
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        ...
        $auditLog->log('login_attempt', 'Invalid CSRF token', 'failed');
    ...
    if ($stmt->num_rows === 1) {
        ...
        if (password_verify($password, $password_hash)) {
            ...
            if ($role_id != ROLE_ADMIN) {
                $rateLimiter->recordAttempt($client_ip, 'login', 'failed');
                $auditLog->log('login_attempt', "Non-admin login attempt: $email", 'failed');
                http_response_code(403);
                echo json_encode(['status' => 'error', 'message' => 'Access denied. Only admins can login here.']);
                exit();
            }
            ...
            $auditLog->log('admin_login', "Email: $email", 'success');
